...
public:
    ...
    static const uint nbp = 5;
    static const uint nbalt_max = 6;
    static const uint ngride = 257;
    static const uint ngridm = 128;
    int PART_SPEC[nbp][ngride];
    int PART_MOM_X[nbp][ngridm];
    int PART_MOM_Y[nbp][ngridm];
    int PART_MOM_Z[nbp][ngridm];
    int counter_up[nbp];
    int counter_down[nbp];
    int counter_total[nbp];

    uint NB_OUTPUT = 0;

    std::vector<record> record_list{};

    bool check_record(const uint &id, const double &momy);

    double get_fraction_RREA_thres(const double potential_MV, const double center_alt_km, const double length_km);

    double get_avalanchge_time(const double potential_MV, const double center_alt_km, const double length_km);
...
private:
    ...
    const uint size_grid_ener = ngride;
    const std::vector<double>
            ENER_GRID{0.0500, 0.0515, 0.0531, 0.0547, 0.0563, 0.0580, 0.0598, 0.0616, 0.0635, 0.0654, 0.0674, 0.0694, 0.0715, 0.0737, 0.0759, 0.0782, 0.0806,
                      0.0830, 0.0855, 0.0881,
                      ...
                      67.8754, 69.9290, 72.0448, 74.2246, 76.4703, 78.7840, 81.1677, 83.6236, 86.1537, 88.7604, 91.4459, 94.2127, 97.0632, 100.0000, 1000000.0};

    const uint size_grid_mom = ngridm;
    const std::vector<double>
            MOM_GRID{-1.0000, -0.9843, -0.9685, -0.9528, -0.9370, -0.9213, -0.9055, -0.8898, -0.8740, -0.8583, -0.8425, -0.8268, -0.8110, -0.7953, -0.7795,
                     -0.7638, -0.7480, -0.7323,
